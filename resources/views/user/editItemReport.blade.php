                ...
                <form action="/updateReportItem/{{$item->id}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-6 text-center">
                            <img id="image" style="width:348px;height:348px"
                        ...
                        <div class="col-md-6">
                            <div class="details">
                                        ...
                                        <select class="form-control" id="item" name="category_id" required disabled>
                                            ...
                                            @if(app()->getLocale()=='ar')
                                                @foreach($categories as $category)
                                                    <option
                                                        {{$category->id==$item->category->id ? 'selected' : '' }} value="{{$category->id}}"> {{$category->category_name_ar}}</option>
                                                @endforeach
                                            @else

                                                @foreach($categories as $category)
                                                    <option
                                                        {{$category->id==$item->category->id ? 'selected' : '' }} value="{{$category->id}}"> {{$category->category_name}}</option>
                                                @endforeach
                                            @endif
                                ...
                                <div class="row">
                                    ...
                                    <div class="col">
                                        <input type="date" class="form-control" id="inputfound_since" name="found_since"
                                               placeholder="Item found when" required value="{{$item->found_since}}">
                                    </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col">
                                        <select class="form-control" id="city" name="city_id" item="{{$item->city->id}}"
                                                required>
                                            <option value="none" selected disabled hidden>
                                                {{ __('messages.Select an Option') }}
                                            </option>
                                            @foreach($cities as $city)
                                                @if(app()->getLocale()=='ar')
                                                    <option
                                                        {{$city->id==$item->city->id ? 'selected' : '' }} value="{{$city->id}}"> {{$city->city_name_ar}}</option>
                                                @else
                                                    <option
                                                        {{$city->id==$item->city->id ? 'selected' : '' }} value="{{$city->id}}"> {{$city->city_name}}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col">
                                        <select name="area_id" id="state" class="form-control">
                                            @foreach($areas as $area)
                                                @if(app()->getLocale()=='ar')
                                                    <option
                                                        {{$area->id==$item->area->id ? 'selected' : '' }} value="{{$area->id}}"> {{$area->area_name_ar}}</option>
                                                @else
                                                    <option
                                                        {{$area->id==$item->area->id ? 'selected' : '' }} value="{{$area->id}}"> {{$area->area_name}}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                    </div>
                                                ...
                                                @if($globalAttributeValue->id==($one->attribute)->id)
                                                    ...
                                                    <div class="col">
                                                        <select class="form-control"
                                                                name="#{{$globalAttributeValue->attribute_name}}"
                                                                id="{{$globalAttributeValue->id}}"
                                                                value="{{$globalAttributeValue->id}}">
                                                            @foreach($globalAttributeValue->values as $v)
                                                                <option
                                                                    {{$v->id==($one->value)->id ? 'selected' : '' }} value="{{$v->id}}">  {{$v->value_name_ar}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                @endif
                                                ...
                                                @if($globalAttributeValue->id==($one->attribute)->id)
                                                    ...
                                                    <div class="col">
                                                        <select class="form-control"
                                                            ...
                                                            @foreach($globalAttributeValue->values as $v)
                                                                <option
                                                                    {{$v->id==($one->value)->id ? 'selected' : '' }} value="{{$v->id}}">  {{$v->value_name}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
